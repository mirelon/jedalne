...
var Table = (function(){
	...
	*/
	...
	table.autofilter = function(t,args) {
		...
		// NA ZACIATKU ZOBRAZ LEN DNESNE JEDLA
                node = document.getElementById('den');
                node.childNodes[2].selectedIndex = 1;
                Table.filter(node.childNodes[2],node.childNodes[2]);
		// NA ZACIATKU ZOBRAZ LEN HLAVNE JEDLA
                node = document.getElementById('typ');
                node.childNodes[2].selectedIndex = 3;
                Table.filter(node.childNodes[2],node.childNodes[2]);
		// NA ZACIATKU TO USORTI PODLA "KDE"
		// TO STACI, ABY MALA TABULKA CLASSU table-autosort:4
	};
